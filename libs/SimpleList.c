...
typedef struct {
	...
	BOOL fLargeBlock;       // use large blocks allocated by VirtualAlloc?
	#endif
...
#if defined(SL_MEM_HEAP)
// Use the default process heap
#include "DefaultHeap.h"
#define SLInternal_Malloc               DefaultHeapAlloc
#define SLInternal_Free                 DefaultHeapFree
#define SLInternal_Realloc              DefaultHeapRealloc

#elif defined(SL_MEM_COM)
// Use the COM memory functions
#include <objbase.h>
#define SLInternal_Malloc               CoTaskMemAlloc
#define SLInternal_Free                 CoTaskMemFree
#define SLInternal_Realloc              CoTaskMemRealloc

#elif defined(SL_MEM_REALLOC)
// Minimize imports: use only realloc
...
HSIMPLELIST SLSAPI SLCreateEx( BOOL fLargeBlock )
{
	...
	if (pHeader)
	{
		...
		pHeader->fLargeBlock = fLargeBlock;
